...
`Brush.Accent` is the logo-orange primary-action, selected-state indicator, link, and progress color; `Brush.AccentStrong` is its hover state. Use `Brush.SelectionSurface` for selected or active backgrounds, `Brush.SurfaceOverlay` for hover and supporting surfaces, and `Brush.TextOnAccent` for text and glyphs placed directly on the vivid accent. Feedback must use semantic brushes: `Success`, `Warning`, or `Danger` with the equivalent `*Surface` brush for a contained status. `Brush.FocusRing` is exclusively for keyboard focus. Never use status colors as decoration or as the sole way to communicate a state.
...
Use `CornerRadius.XS` (4 px) for small internal elements, `S` (6 px) for buttons, inputs, badges, tags, and switches, `M` (8 px) for cards, `L` (12 px) for dialogs, `XL` (16 px) for prominent feature surfaces, and `Pill` only for slim indicators.
...
Components must have default, hover, keyboard-focus, disabled, and (where relevant) selected, error, and loading states. Preserve a 32 px minimum hit target; use 40 px for icon-only controls and comfortable text inputs. Keep the primary action visually unique, communicate disabled states with both opacity and unavailable behavior, and use `Brush.FocusRing` for custom inputs and switches; shared buttons and toggle buttons intentionally retain their normal border after mouse focus.
